...
void loop() {
  ...
  if (cmd.params[0] == 2) {
    ...
    if (nicla_flag != 0) {// checks if new yaw occurs
        ...
        //Serial.print("Dectected!");
        behave.params[0] = cmd.params[0]; // flag
        behave.params[1] = -0.3;//cmd.params[1]; // fx ('meters'/second)
      ...
      // Serial.print("No detected~");
        ...
        unsigned long last_rotation_time = 0; // last rotation time
        unsigned long current_time = millis();
        // Check whether meet the time period
        if (current_time - last_rotation_time >= 5000){
          nicla_yaw = _yaw + 0.25;
          if (nicla_yaw > 6.28319){
            nicla_yaw -= 6.28319;
          }
          last_rotation_time = current_time;
        }
      behave.params[0] = cmd.params[0]; // flag
      behave.params[1] = -0.05;//cmd.params[1]; // fx ('meters'/second)
      behave.params[2] = cmd.params[2]; // fz (meters)
      behave.params[3] = 0; // tx (radians/second)
      behave.params[4] = nicla_yaw; // tz (radians)
    } 
    ...
    // behave.params[0] = cmd.params[0]; // flag
    // behave.params[1] = -0.2;//cmd.params[1]; // fx ('meters'/second)
    // behave.params[2] = cmd.params[2]; // fz (meters)
    // behave.params[3] = 0; // tx (radians/second)
    // behave.params[4] = nicla_yaw; // tz (radians)
